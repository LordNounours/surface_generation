...
import numpy as np
from utils.data_utils import read_csv_files, read_information_file, process_curves 
from utils.utils import compute_z_min_max,create_color_palettes,get_color_from_z_leaves,check_memory_usage
...
def generate_cylinder(point_start, point_end, radius):
    ...
    amount_value=8
    dynamic_sides = int(min_sides + (radius * amount_value))  
...
def generate_sphere(point, radius, number_of_points=50):
    """
...
def generate_surface_root(points,dims,filename,config) :
    ...
    writer.SetFileName(f"/home/caro/Documents/Ressources/Annotation_plante_01/mesh/leaf/test_cylinder/iso_surface_cylinder{filename}.vtk")  # Fichier unique par racine
...
def generate_surface_stem(points,dims,color,filename,config) :
    ...
    writer = vtk.vtkPolyDataWriter()
    writer.SetFileName(f"/home/caro/Documents/Ressources/Annotation_plante_01/mesh/leaf/test_stem/iso_surface_stem{filename}.vtk")  # Fichier unique par racine
    writer.SetInputData(isoSurface)
    writer.Write()
...
def process_leaves(curve_points_dict_roots, all_leaves_csv_files, information, config, z_min_roots, z_max_roots,dims,palettes):
    ...
    for i, (filename,dir_index) in enumerate(all_leaves_csv_files):
        levels_dict[filename] = [level  for level in information[i]]
    for i, (filename,dir_index) in enumerate(all_leaves_csv_files):
        ...
        curve_color = get_color_from_z_leaves(curve_points[0][0][2], z_min_roots, z_max_roots,dir_index,palettes)
        print(curve_color)

        # Geometry Generation
        x_s, y_s, z_s = [coord for coord in curve_points[0][0]]
        for j, point in enumerate(curve_points[0]):
            x, y, z = [coord  for coord in point]
            if(z<levels[0]) :
                radius=config.leaf_small_radius 
            elif(z<levels[1]) :
                radius=config.leaf_mid_radius
            else :
                radius=config.leaf_big_radius
            radius_stem = 5
            sphere_points = generate_sphere([x_s, y_s, z_s], radius_stem)
...
def main(config):
    ...
    n_dir = max(i for _, i in all_leaves_csv_files)
    palette=create_color_palettes(n_dir)
    ...
    palette=create_color_palettes(len(all_leaves_csv_files))
    process_leaves(curve_points_dict_leaves, all_leaves_csv_files, information, config, z_min_leaves, z_max_leaves,dims,palette)
